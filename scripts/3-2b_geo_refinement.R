...
## species native distribution
  s.nd   <- gts_list$native_distribution[gts_list$taxon==gsub("_"," ", f.nm)]
## species native distribution list
  s.nd.l <- unlist(strsplit(s.nd, "; "))
...
#######################################
  ...
  ## flag records where GTS country doesn't match the coordinates and then output into folder
    ...
    eo.post <- eo.post %>% mutate(ISO0_match0=(ifelse(NAME_0 %in% s.nd.l, TRUE, FALSE)))
  ...
  ## flag records where GTS country doesn't given dataset country
    eo.post <- eo.post %>% mutate(ISO0_match2=(ifelse(NAME_0 %in% s.nd.l, TRUE, FALSE)))
...
#######################################
  ...
  # ## flag records where GTS state (adm1) doesn't match the coordinates and then output into folder
    ...
    eo.post <- eo.post %>% mutate(ISO1_match0=(ifelse(NAME_1 %in% s.nd.l, TRUE, FALSE)))
