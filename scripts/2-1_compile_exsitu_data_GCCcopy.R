...
all_data5$genus_new <- mgsub(all_data5$genus_new,
  c(#"\"",
    ...
    "^Q$","Querucs","Cyclobalanopsis"),#"Quercu","^Q.$","Querces","Qurercus","Quercuss","Querus",
  c(#"",
...
target_genera <- c("Malus","Pyrus","Quercus","Sinomalus","Tilia","Ulmus")
  #,"Acer","Magnolia","Kalopanax","Dipteronia",
  #"Cyclobalanopsis","Michelia","Manglietia")"Microptelea")
all_data6 <- all_data5 %>% filter(genus_new %in% target_genera)
nrow(all_data6) #38252
# see records removed:
...
taxon_list <- taxon_list %>%
  select(taxon_name,taxon_name_acc,species_name_acc,list,rl_year,rl_category)#threat_category,threat_assessment)
#taxon_list <- read.csv(file.path(main_dir, "inputs", "taxa_list",
...
all_data10 <- rbind(matched,need_match)
  table(all_data10$list) # desiderata: 30806 | synonym: 545 ; 3/17/21: 29960, 312
  # see how many rows have taxon name match
nrow(all_data10[which(!is.na(all_data10$list)),]) #31351 ; 30272
