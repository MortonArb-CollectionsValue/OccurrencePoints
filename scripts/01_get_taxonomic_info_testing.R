...
### INPUTS:
  # target_taxa.csv (list of target taxa)
    # columns:
      # 1. "target_taxa.csv" (genus, species, infra rank, and infra name, all
      #    separated by one space each; hybrid symbol should be " x ", rather
      #    than "_" or "✕", and go between genus and species; column should be
      #    named "taxon_name_acc")
...
# read in taxa list
taxa_list_acc <- read.csv(file.path(imls.meta, "target_taxa.csv"),
...
datasets <- list(tp_syn_df,itis_syn_df,pow_syn_df)
all_synonyms <- Reduce(rbind.fill,datasets)
all_syn <- all_synonyms
  names(all_syn)
  str(all_syn)

# add a space after every period and fix some other inconsistencies,
#  to standardize authors more
all_syn$match_name_with_authors <- gsub(".",". ",
  all_syn$match_name_with_authors,fixed=T)
all_syn$match_name_with_authors <- str_squish(
  all_syn$match_name_with_authors)
all_syn$match_name_with_authors <- gsub(". )",".)",
  all_syn$match_name_with_authors,fixed=T)
all_syn$match_name_with_authors <- gsub("(pro sp.)","",
  all_syn$match_name_with_authors,fixed=T)
# replace accented characters
all_syn$match_name_with_authors <- stringi::stri_trans_general(
  all_syn$match_name_with_authors, "Latin-ASCII")

# keep unique values and create
#   "ref" col of all databases with duplicates,
#   "status" col of all acceptance statuses of duplicates,
#   "ref_id" col with id numbers from matching names
all_syn$status_standard <- as.character(all_syn$acceptance)
  unique(all_syn$status_standard)
all_syn <- setorder(all_syn,status_standard)
all_syn <- all_syn %>%
...
add <- setDT(all_syn)[, list(ref= toString(sort(unique(strsplit(ref,
  ',\\s*|\\s+')[[1]])))), by = ref_id]
all_syn <- subset(all_syn, select=-ref)
all_syn <- join(all_syn,add)
unique(all_syn$ref)
unique(all_syn$status)
# add "ref_count" column tallying number of items (databases) per taxon
all_syn$ref_count <- str_count(all_syn$ref, ',')+1
all_syn[which(all_syn$ref == "NA"),]$ref_count <- 0
str(all_syn)
...
all_syn <- all_syn %>% separate("taxon_name_match",
  ...
  remove=F,fill="right")
all_syn$genus_species_match <- paste(all_syn$genus,all_syn$species,sep=" ")
# separate out taxon_name_acc
all_syn <- all_syn %>% separate("taxon_name_acc",
  c("genus_acc","species_acc"),sep=" ",extra="warn",remove=F,fill="right")
all_syn$genus_species_acc <- paste(all_syn$genus_acc,all_syn$species_acc,
  sep=" ")
# order rows
all_syn <- setorderv(all_syn,c("taxon_name_match","ref_count","ref",
  "status_standard"),c(1,-1,-1,1))
# write file
#write.csv(all_syn,file.path(imls.output, "taxize_all_synonyms.csv"),
#  row.names=FALSE)

# IF DESIRED:
  # remove forms
nrow(all_syn)
all_syn <- all_syn[which(is.na(all_syn$infra_rank) |
  all_syn$infra_rank != "f."),]
nrow(all_syn)
  # remove synonyms with less than two sources
all_syn <- all_syn[which(all_syn$ref_count > 1 |
  grepl("homotypic",all_syn$status_standard) |
  grepl("accepted",all_syn$status_standard)),]
nrow(all_syn)
  # remove records where same syn match name goes with more than 1 taxon_name_acc
all_syn$dup <- c(duplicated(all_syn$taxon_name_match,fromLast=T)
  | duplicated(all_syn$taxon_name_match))
all_syn <- setdiff(all_syn,all_syn[which(
  all_syn$status_standard == "synonym" & all_syn$dup == T),])
nrow(all_syn)
  # remove var. and subsp. records with species name already accounted for
#all_syn <- setdiff(all_syn,all_syn[which(
#  !is.na(all_syn$infra_rank) &
#  all_syn$genus_species_match == all_syn$genus_species_acc &
#  grepl("synonym",all_syn$status_standard)),])
#nrow(all_syn)
  # remove var. and subsp. for accepted taxa (children)
#all_syn <- setdiff(all_syn,all_syn[which(
#  !is.na(all_syn$infra_rank) &
#  grepl("synonym",all_syn$status_standard)),])
#nrow(all_syn)
...
# create version to use in next scripts
unique(all_syn$status_standard)
all_syn <- all_syn %>%
  mutate(list = recode(status_standard,
         "homotypic_synonym" = "synonym",
         "accepted" = "desiderata",
         "accepted,synonym" = "synonym"
       )) %>%
  dplyr::select(taxon_name_acc,taxon_name_match,list,ref)
head(all_syn)
# write file
write.csv(all_syn,file.path(imls.output, "target_taxa_with_syn.csv"),
  row.names=FALSE)
...
itis_all <- rbind.fill(itis_names_noDup,itis_syn_df)
# look at duplicates
#itis_all[which(duplicated(itis_all$taxon_name_acc) &
...
#families <- c("Juglandaceae","Fagaceae","Lauraceae","Magnoliaceae","Pinaceae","Taxaceae")
...
tpl_names$taxon_name_acc <- mgsub(tpl_names$taxon_name_acc,
  c("NA "," NA"," NA"," NA"," NA"), "")
...
colnames(tpl_names)
# keep only necessary columns
tpl_names <- tpl_names[,c("taxon_name_acc","taxon_name_match","author","match_id",
  "database","acceptance","match_name_with_authors","family","source")]

# remove duplicates
tpl_names_noDup <- tpl_names
tpl_names_noDup$dup <- c(duplicated(tpl_names_noDup$taxon_name_acc,fromLast = TRUE)
  | duplicated(tpl_names_noDup$taxon_name_acc))
...
tpl_all <- tpl_names_noDup %>% filter(tpl_names_noDup$taxon_name_acc %in%
  taxa_names)
# write file
write.csv(tpl_all,file.path(imls.raw, "taxize_tpl.csv"),row.names=FALSE)
...
#genera <- c("Carya","Fagus","Gymnocladus","Juglans","Lindera","Magnolia","Persea","Pinus","Sassafras","Taxus")
...
ipni_names$taxon_name_acc <- gsub(" × "," x ",
  ipni_names$taxon_name_acc,fixed=T)
# fill other columns
ipni_names$taxon_name_match <- ipni_names$taxon_name_acc
ipni_names$match_name_with_authors <- paste(ipni_names$taxon_name_acc,
  ipni_names$author)
...
ipni_names_noDup <- distinct(ipni_names_noDup,taxon_name_acc,.keep_all=T)
colnames(ipni_names_noDup)
# keep only necessary columns
ipni_names_noDup <- ipni_names_noDup[,c("taxon_name_acc","taxon_name_match","author",
  "match_id","database","match_name_with_authors","family")]

# join with taxa list and remove non-matches
ipni_all <- ipni_names_noDup %>% filter(ipni_names_noDup$taxon_name_acc %in%
  taxa_names)
# write file
write.csv(ipni_all,file.path(imls.raw, "taxize_ipni.csv"),row.names=FALSE)
...
#tp_all <- read.csv(file.path(imls.raw, "taxize_tropicos.csv"),header=T,
#  na.strings=c("","NA"),colClasses="character"); nrow(tp_all)
#itis_all <- read.csv(file.path(imls.raw, "taxize_itis.csv"),header=T,
#  na.strings=c("","NA"),colClasses="character"); nrow(itis_all)
#tpl_all <- read.csv(file.path(imls.raw, "taxize_tpl.csv"),header=T,
#  na.strings=c("","NA"),colClasses="character"); nrow(tpl_all)
#ipni_all <- read.csv(file.path(imls.raw, "taxize_ipni.csv"),header=T,
#  na.strings=c("","NA"),colClasses="character"); nrow(ipni_all)
...
# fill "NA" taxon_name_acc with accepted name
all_names[which(is.na(all_names$taxon_name_acc)),]$taxon_name_acc <-
  all_names[which(is.na(all_names$taxon_name_acc)),]$taxon_name_acc
# For IMLS dataset:
# fill taxon_name_acc col for cultivars
#all_names[which(all_names$taxon_type == "cultivar"),]$taxon_name_acc <-
#  all_names[which(all_names$taxon_type == "cultivar"),]$taxon_name_acc
# fill taxon_name_acc column for hybrid_no_x taxa
...
all_data <- setorderv(all_data,c("taxon_name_match","ref_count","ref",
  "status_standard"),c(1,-1,-1,1))
# write file
#write.csv(all_data,file.path(imls.output, "taxize_all_names_raw.csv"),
#  row.names=FALSE)
...
nrow(all_data2)
  # remove var. and subsp. records with species name already accounted for
all_data2 <- all_data2 %>% separate("taxon_name_acc",
  c("genus_acc","species_acc"),sep=" ",extra="warn",remove=F,fill="right")
all_data2$genus_species_acc <- paste(all_data2$genus_acc,all_data2$species_acc,
  sep=" ")
all_data2 <- setdiff(all_data2,all_data2[which(
  !is.na(all_data2$infra_rank) &
  all_data2$genus_species_match == all_data2$genus_species_acc &
  all_data2$status_standard == "synonym"),])
nrow(all_data2)
  # remove var. and subsp. synonyms with only one source
all_data2 <- setdiff(all_data2,all_data2[which(
  !is.na(all_data2$infra_rank) &
  all_data2$status == "synonym"),])
nrow(all_data2)
  # remove var. and subsp. for accepted taxa (children)
all_data2 <- setdiff(all_data2,all_data2[which(
  !is.na(all_data2$infra_rank) &
...
all_data2 <- setdiff(all_data2,all_data2[which(
  !is.na(all_data2$infra_rank) &
  all_data2$genus_species_match %in% all_data2$genus_species_acc &
  all_data2$status_standard == "synonym"),])
nrow(all_data2)
  # remove records where same syn match name goes with more than 1 taxon_name_acc
all_data2$dup <- c(duplicated(all_data2$taxon_name_match,fromLast=T)
...
# final ordering of names
all_data3 <- as.data.frame(setorder(all_data2,taxon_name_match))
# write file
write.csv(all_data3,file.path(imls.output, "taxize_all_names.csv"),
  row.names=FALSE)
...
setnames(tnrs_names,
  ...
  new = c("taxon_name_acc","taxon_name_match","source", "author","match_id"),
  skip_absent=T)
...
# keep only necessary columns
tnrs_all <- tnrs_all[,c("taxon_name_acc","taxon_name_match","author","match_id",
