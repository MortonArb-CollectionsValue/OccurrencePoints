...
## OPTIONAL: read in county-lvel native distribution data
native_dist2 <- read.csv(file.path(main_dir,"inputs","known_distribution",
  "GA2_BONAP_June2018.csv"), header = T, na.strings = c("","NA"),
  colClasses = "character")
native_dist2 <- native_dist2 %>%
  separate("FIPS",into=c("STATEFP","COUNTYFP"),sep=2,remove=F) %>%
  filter(is.na(questionable) & is.na(adventive)) %>%
...
taxon_list <- read.csv(file.path(main_dir,"inputs","taxa_list",
  ...
  colClasses = "character")
taxon_list <- taxon_list %>% dplyr::select(species_name_acc,taxon_name)
native_dist2 <- left_join(native_dist2,taxon_list)
native_dist2 <- left_join(native_dist2,adm2.poly@data)
native_dist2 <- native_dist2 %>%
  group_by(species_name_acc) %>%
  summarise(bonap_native_dist = paste(COUNTYNS,collapse="; ")) %>%
  ungroup() %>% filter(!is.na(species_name_acc))
...
summary_tbl <- data.frame(species_name_acc = "start", total_pts = "start",
  #unflagged_pts = "start",
  .cen = "start", .urb = "start", .inst = "start",
  .con = "start", .outl = "start", .gtsnative = "start", .rlnative = "start",
   .bonapnative = "start",#.rlintroduced = "start",
  .yr1950 = "start", .yr1980 = "start", .yrna = "start", stringsAsFactors=F)
  ...
  c.nms <- c("species_name_acc", "taxon_name", "scientificName",
    ...
    "STATEFP","COUNTYFP","COUNTYNS","NAME",
    ".cen",".urb",".inst",".con",".outl",".gtsnative",".rlnative",
    ".bonapnative",".yr1950",".yr1980",".yrna")#".rlintroduced",
...
for (i in 1:length(spp_list)){
  ...
  eo.spdf2 <- SpatialPointsDataFrame(eo.post[,c("decimalLongitude",
    "decimalLatitude")], eo.post, proj4string = CRS(proj4string4poly))
  ## add county data
  adm2.poly <- spTransform(adm2.poly,proj4string4poly)
  eo.post <- point.in.poly(eo.spdf2, adm2.poly, sp=TRUE)@data
  ...
  ## CHECK POINT LOCATION AGAINST "ACCEPTED" COUNTY DISTRUBUTION
  ## BONAP native counties
  s.nd.bon.l <- unique(unlist(strsplit(native_dist2$bonap_native_dist[
    native_dist2$species_name_acc==gsub("_"," ", f.nm)], "; ")))
  if(!is.na(s.nd.bon.l)){
  ## flag records where BONAP native county does match record's coord location
  eo.post <- eo.post %>% mutate(.bonapnative=(ifelse(
    COUNTYNS %in% s.nd.bon.l, TRUE, FALSE)))
  } else {
    eo.post$bonapnative <- NA
  }
  ...
  summary_add <- data.frame(
    ...
    .bonapnative = sum(!eo.post3$.bonapnative),
